...
// const finalArr = deals.filter( deal  =>
//   leads.some(lead => lead.name === deal.name)
// )
...
// , "Status", "Week","Parent_Id.Billing_Rate",
const sortOrder = [ "Status", "Week", "id", "Notes"];

function recursiveFilter(firstPatient, secondPatient, filterIndex) {
  var nameA = firstPatient[`${sortOrder[filterIndex]}`]?.toString().toUpperCase(); // ignore upper and lowercase
  var nameB = secondPatient[`${sortOrder[filterIndex]}`]?.toString().toUpperCase(); // ignore upper and lowercase
  if (nameA < nameB) {
    return -1;
  } else if (nameA > nameB) {
    return 1;
  } else {
    return filterIndex==sortOrder.length-1?1:recursiveFilter(firstPatient, secondPatient, ++filterIndex)
  }
}

if(sortOrder.length)
patients.sort((firstPatient, secondPatient) => {
  return recursiveFilter(firstPatient, secondPatient, 0);
});

patients.map((patient) =>
  console.log({
    name: patient[`Parent_Id.Name`],
    Billing_Rate: patient[`Parent_Id.Billing_Rate`],
